...
if (!isNil "_loadout" && {!isNil "phx_bluforBaseChannel"} && {!isNil "phx_opforBaseChannel"} && {!isNil "phx_indforBaseChannel"} && {isNil "phx_hasSWitem"} && {!isNil "phx_groupIDset"} && {isNil "phx_isSpectator"}) then {
    phx_hasSWitem = false;
    phx_hasLRitem = false;
