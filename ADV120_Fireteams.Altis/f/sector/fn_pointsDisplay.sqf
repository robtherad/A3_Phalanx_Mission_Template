...
private _totalCapTime = ["phx_sector_sectorTime",.1] call BIS_fnc_getParamValue;
...
[{
    ...
    if (phx_show_timeUI) then {
        ...
        } forEach [["phx_sector_westPointsString", phx_sector_westPointsPublic, "BLUFOR"], ["phx_sector_eastPointsString", phx_sector_eastPointsPublic, "OPFOR"], ["phx_sector_indPointsString", phx_sector_indPointsPublic, "INDFOR"]];

        // Create displays in bottom left
        ("bluRsc" call BIS_fnc_rscLayer) cutRsc ["bluforStructText", "PLAIN"];
        ("redRsc" call BIS_fnc_rscLayer) cutRsc ["opforStructText", "PLAIN"];
        ("indRsc" call BIS_fnc_rscLayer) cutRsc ["indforStructText", "PLAIN"];

        // Update text in the displays to match the points markers
        private _display = uiNameSpace getVariable "opforStructText";
        ...
        private _display3 = uiNameSpace getVariable "indforStructText";
        private _setText3 = _display3 displayCtrl 1003;
        _setText3 ctrlSetStructuredText (parseText format ["%1",phx_sector_indPointsString]);
    } else {
        ("bluRsc" call BIS_fnc_rscLayer) cutText ["", "PLAIN"];
        ("redRsc" call BIS_fnc_rscLayer) cutText ["", "PLAIN"];
        ("indRsc" call BIS_fnc_rscLayer) cutText ["", "PLAIN"];
    };
